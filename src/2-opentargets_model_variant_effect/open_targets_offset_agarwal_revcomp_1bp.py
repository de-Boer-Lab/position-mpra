...
def main(model_path, output_path):
    ...
    def generate_sequences(row, sidemer_offset=0):
        ...
        # Convert to Seq objects to reverse complement
        ref_seq_rc = str(Seq(ref_sequence).reverse_complement())
        alt_seq_rc = str(Seq(alt_sequence).reverse_complement())
        ...
        return {'Ref_Sequence': str(ref_seq_rc), 'Alt_Sequence': str(alt_seq_rc)}
    ...
    k562 = pd.read_csv("/scratch/st-cdeboer-1/sambina/mpra/data/chromosome/agarwal/data_k562/fold_0/valid.txt", sep="\t")
