        ...
        with open(f"{output}", 'w') as f:
            ...
            for _, row in tqdm(test_df.iterrows(), total=len(test_df)):
                ...
                if len(row["random_seq"]) != 231:
                    encoded_seq_ref = one_hot_encode(upstream+row['random_seq']+downstream)
                else:
                ...
                if "motif_name" in test_df.columns:
                    f.write(f"{row['seq_id']},{row['motif_name']},{upstream+row['random_seq']+downstream},{pred_value_ref[0]}\n")
                else:
                    f.write(f"{row['seq_id']},{upstream+row['random_seq']+downstream},{pred_value_ref[0]}\n")
...
def plot(df):
    ...
    plt.title("Box Plot of Random Sequence Predictions")
    # Show the plot
    plt.show()
